...
import io.github.ngraciano.libraryapi.security.CustomAuthentication;
...
public class AuthorizationServerConfiguration {
    ...
    private RSAKey generateKeyRSA() throws Exception {
        ...
        return new RSAKey.Builder(keyPublic).privateKey(keyPrivate).keyID(UUID.randomUUID().toString()).build();
    }
    ...
    public AuthorizationServerSettings authorizationServerSettings(){
        return AuthorizationServerSettings.builder().tokenEndpoint("/oauth2/token").tokenIntrospectionEndpoint("/oauth2/introspect")
    ...
    public OAuth2TokenCustomizer<JwtEncodingContext> tokenCustomizer(){
        return context -> {
            ...
            if (principal instanceof CustomAuthentication authentication){
                OAuth2TokenType typeT=context.getTokenType();
                if (OAuth2TokenType.ACCESS_TOKEN.equals(typeT)){
                    Collection<GrantedAuthority> authorities=authentication.getAuthorities();
                    List<String> authoritiesList = (List<String>) authorities.stream().map(GrantedAuthority::getAuthority).toList();
                    context.getClaims().claim("authorities",authoritiesList).claim("email",authentication.getUser().getEmail());
                }
            }
        };
    }
